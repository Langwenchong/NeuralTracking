...
def warp_point_with_nodes(node_positions, nodes_rotation, nodes_translation, pos_x, pos_y, pos_z):
	...
	new_x += node_positions[0] + nodes_translation[0]
	new_y += node_positions[1] + nodes_translation[1]
	new_z += node_positions[2] + nodes_translation[2]
...
class WarpField:
	def __init__(self, graph_dict, tsdf, use_gpu=True, kdtree_leaf_size=16):
		...
		self.update_kdtree(graph_dict["graph_nodes"])
		...
		self.graph_neighbours = graph_dict["graph_neighbours"]

		self.use_gpu = use_gpu	

	def update(self,graph_data):
		graph_data["graph_nodes"] = graph_data["graph_nodes"].astype(np.float32)
		self.graph_dict = graph_data
		self.update_kdtree(graph_data["graph_nodes"])
		self.graph_neighbours = graph_data["graph_neighbours"]


	def update_kdtree(self,points):
		self.kdtree = KDTreeCPU(points, leafsize=self.kdtree_leaf_size)
	...
	@staticmethod	
	@ncuda.jit()
	def deform_world_points_lbs_cuda(deformed_pos, volume, anchors, weights, rotations, translations, positions):
		x, y = ncuda.grid(2)
			...
			if weights_sum <= 1e-6:
				...
				for i in range(anchors.shape[3]):
					if weights[x,y,z,i]/weights_sum == 0:
						continue
					new_x, new_y, new_z = warp_point_with_nodes(positions[anchors[x, y, z, i]],
																rotations[anchors[x, y, z, i]],
	...
	def deform_tsdf(self, rotations, translations, points=None, method='dqs'):
		...
		if points is None:  # If not points are provided run on all voxels
			...
			reshape_gpu_vol = [points.shape[0],1,1]
		...
		world_anchors = world_anchors.astype(np.int32)	
		world_weights = world_weights.astype(np.float32)	
		...
		if self.use_gpu:
			...
			world_anchors = world_anchors.reshape(reshape_gpu_vol+[self.graph_neighbours])
			world_weights = world_weights.reshape(reshape_gpu_vol+[self.graph_neighbours])
			deform_world_pts = np.ascontiguousarray(np.zeros_like(points))
			...
			self.deform_world_points_lbs_cuda[blockspergrid, (threadsperblock)](deform_world_pts, points, 
			world_anchors, world_weights, 
			rotations, translations, 
			self.graph_dict["graph_nodes"].astype(np.float32))
			ncuda.synchronize() 
			deform_world_pts = deform_world_pts.reshape(-1,3)
		
		else:
			if method == "lbs":  # For Linear blend skinning
				deform_world_pts = self.deform_world_points_lbs(rotations, translations,
				                                                points,
				                                                world_anchors, world_weights,
				                                                self.graph_dict["graph_nodes"].astype(np.float32))
			elif method == "dqs":  # For Dual Quaternion skinning
				deform_world_pts = self.deform_world_points_dqs(quaternions, translations,
				                                                points,
				                                                world_anchors, world_weights,
				                                                self.graph_dict["graph_nodes"].astype(np.float32))
			else:
				raise NotImplementedError("Method:{} is not implemented.\
				 Kindly use Linear Blend Skinning:lbs or Dual Quaternion Skinning:dqs")

		valid_points = np.sum(world_weights,axis=1) > 1e-6
		
		wated_graph_nodes = np.unique(world_anchors)[1:] # Remove -1
		# unwanted_graph_nodes =

		return deform_world_pts,valid_points
...
if __name__ == "__main__":
	...
	# tsdf = TSDFVolume(max_depth, voxel_size, cam_intr, graph_dict, use_gpu=True)
